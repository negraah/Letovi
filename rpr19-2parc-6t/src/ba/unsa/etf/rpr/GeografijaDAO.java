...
public class GeografijaDAO {
    ...
    private PreparedStatement glavniGradUpit, dajDrzavuUpit, obrisiDrzavuUpit, obrisiGradoveZaDrzavuUpit, nadjiDrzavuUpit,
            ...
            nadjiGradUpit, obrisiGradUpit, dajDrzaveUpit,dodajLetUpit, odrediIdLetaUpit, obrisiLetUpit, nadjiLetUpit, nadjiGradUpitId;
    ...
    private GeografijaDAO() {
        ...
        try {
            ...
            nadjiGradUpitId = conn.prepareStatement("SELECT * FROM grad WHERE id=?");
            ...
            obrisiLetUpit = conn.prepareStatement("DELETE FROM letovi WHERE id1=? or id2=?");
            nadjiLetUpit = conn.prepareStatement("SELECT * FROM letovi WHERE id1=? or id2=?");
    ...
    public ArrayList<Grad> gradovi() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                rezultat.add(nadjiGrad(grad.getNaziv()));
    ...
    public void dodajGrad(Grad grad) {
        try {
            ...
            if(grad.getLetovi()!=null) {
                for (Grad grad1 : grad.getLetovi()) {
                    dodajLetUpit.setInt(1, grad.getId());
                    dodajLetUpit.setInt(2, grad1.getId());
    ...
    public void izmijeniGrad(Grad grad) {
        try {
            ...
            //obrisi sve koji su povezani sa tim gradom i onda dodaj samo veze
            obrisiLetUpit.setInt(1, grad.getId());
            obrisiLetUpit.setInt(2, grad.getId());
            obrisiLetUpit.executeUpdate();
    ...
    public Grad nadjiGrad(String nazivGrada) {
        try {
            ...
            TreeSet<Grad> letovi1 = new TreeSet<>();
            nadjiLetUpit.setInt(1, grad.getId());
            nadjiLetUpit.setInt(2, grad.getId());
            //ima dva id u sebi rs
            rs = nadjiLetUpit.executeQuery();

            while(rs.next()){
                if(grad.getId()==rs.getInt(2)) {
                    nadjiGradUpitId.setInt(1, rs.getInt(1));
                }else {
                    nadjiGradUpitId.setInt(1, rs.getInt(2));
                }
                //svi podaci o gradu
                ResultSet rs1 = nadjiGradUpitId.executeQuery();
                Drzava d1 = dajDrzavu(rs1.getInt(4));
                Grad grad1 = dajGradIzResultSeta(rs1, d1);
                letovi1.add(grad1);
            }

            grad.setLetovi(letovi1);
            return grad;
        } catch (SQLException e) {
            e.printStackTrace();
            return null;
